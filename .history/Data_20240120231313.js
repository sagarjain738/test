...
export function getTotal(data) {
  ...
  data.salesData.forEach((product) => {
    product.sales.forEach((sale) => {
      sumOfMonths[sale.month] = (sumOfMonths[sale.month] || 0) + sale.amount;
    });
  });

  const output = [];

  for (const key in sumOfMonths) {
    output.push({
      id: key,
      label: key,
      value: sumOfMonths[key],
      color: "hsl(37, 70%, 50%)",
    });
  }
...
export function getMonths() {
  ...
  months.add("name");
  data.salesData.forEach((d) => d.sales.forEach((v) => months.add(v.month)));
